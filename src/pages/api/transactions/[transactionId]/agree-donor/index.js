...
export default async function handler(req, res) {
    ...
    if (req.method === 'PUT') {
      try {
        ...
        if (existingTransaction.agreedByDonor) {
          return res.status(200).json(existingTransaction);
        }
        console.log("existingTransaction.agreedByDonor: ", existingTransaction.agreedByDonor);
        // Update the agreedByDonor field
        const updatedTransaction = await TransactionModel.findByIdAndUpdate(
          transactionId,
          { $set: { agreedByDonor: true } },
          { new: true } 
        );
  
        // Check if status needs to be updated
        const transaction = await updateStatusIfAgreed(updatedTransaction);
