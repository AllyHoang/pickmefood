            ...
            <div className="flex flex-col justify-between space-y-2">
              {MenuList.map((item) => (
                <NavigationMenuItem key={item.label}>
                  ...
                  <NavigationMenuLink
                  className={`relative pl-4 flex items-center gap-3 h-12 w-full text-xl p-3 cursor-pointer rounded-lg ${isActive(item.href) ? 'bg-sky-200' : 'text-gray-700 hover:bg-sky-100'} `}
                >
                  {/* Left bar indicator */}
                  ...
                  <NavigationMenuContent className="">
                    <ul className="grid gap-3 p-6 md:w-[100px] lg:w-[190px] lg:grid-cols-[1fr]">
                      <Link href="/about" legacyBehavior passHref>
                        <a className={`${navigationMenuTriggerStyle()} w-[150px] h-12 text-md flex items-center gap-2`}>
                          <RxRocket size="20px" />
                          About Us
                        </a>
                      </Link>
                      ...
                      <a href="#" onClick={handleLogout} className={`${navigationMenuTriggerStyle()} w-[140px] h-12 text-md flex items-center gap-2`}>
