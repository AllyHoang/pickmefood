      ...
      <div className="grid grid-cols-3 items-center gap-4 mb-5">
        <div className="col-span-3 md:col-span-2">
          <div>
            ...
            <h1 className="flex-auto pt-3 text-base text-gray-500"> Check out your top matches donation and requests below</h1>
          </div>
        </div>
            ...
            <div className="overflow-hidden rounded-md bg-white p-6 shadow-lg">
              <div className="flex items-center md:flex-col xl:flex-row">
                ...
                <span className="my-1 inline-flex grow flex-row-reverse rounded-md">
                  <button className="inline-flex items-center rounded-md bg-sky-100 px-3 py-2 text-sm font-medium leading-4 text-sky-500 hover:bg-sky-200 focus:outline-none">Gain more points!</button>
                </span>
              </div>
            </div>
          </div>
        ...
        {filteredItems.map((donation) => (
          ...
          >
            <CardHeader className="flex-col gap-4 items-start">
              <Badge
                variant={`${donation.type === "Request" ? "primary" : "secondary"}`}
                className={`px-3 py-1 rounded-full text-sm font-large text-s ${donation.type === "Request" ? "bg-sky-100" : "bg-emerald-100"}`}
              >
            ...
            <CardFooter className="flex justify-between">
              ...
              <Drawer direction="right">
                <DrawerTrigger asChild>
                  <Button onClick={() => setSelectedDonation(donation)} className="">View Details</Button>
                </DrawerTrigger>
                <DrawerContent
                ...
                <div className="p-4 bg-white shadow-lg rounded-lg">
                    {selectedDonation?.image && (
                        <div className="overflow-hidden rounded-t-lg">
                            <img src={`/images/${selectedDonation?.image}`} alt="Donation" className="w-full object-cover hover:scale-105 transition-scale duration-300" style={{ height: '200px' }} />
                        </div>
                    )}
                    <div className="p-4">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className={`text-2xl text-heading4-bold font-bold  ${donation.type === "Request" ? "text-sky-600" : "text-emerald-600"}`}>{selectedDonation?.title}</h2>
                        </div>
                        <p className="text-gray-500">{selectedDonation?.description}</p>
                        <div className="mt-4">
                            <div className="flex justify-between align-middle">
                              <p className="flex items-center text-gray-800 font-bold">
                                  <RxPerson className="mr-2" size="20px" />
                                  {selectedDonation?.type === 'Request' ? "Requester" : "Donor"}: 
                              </p>
                              <span>{selectedDonation?.ownerName}</span>
                            </div>
                            <div className="flex justify-between align-middle ">
                            <p className="flex items-center text-gray-800 font-bold">
                                <RxSewingPin className="mr-2" size="20px" /><span className="font-bold">Location:</span>
                            </p>
                            ...
                            {selectedDonation?.expiryDate && (
                              <div className="flex justify-between align-middle ">
                                <p className="flex items-center text-gray-800 font-bold">
                                    <RxClock className="mr-2" size="20px" />Expires: 
                                </p>
